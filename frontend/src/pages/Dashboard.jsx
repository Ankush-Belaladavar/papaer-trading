...
const Dashboard = () => {
  ...
  const checkWatchlistStatus = async (symbol) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:5000/api/watchlist/check/${currentUser.id}/${symbol}`
      );
  ...
  const fetchStockChartData = async (symbol, newRange = range) => {
    ...
    try {
      ...
      const yahooFinanceUrl = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(symbol)}?interval=1d&range=${newRange}&t=${Date.now()}`;
