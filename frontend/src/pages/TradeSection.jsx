...
const TradeSection = ({ stock }) => {
  ...
  const fetchStockDetails = async (symbol) => {
    ...
    try {
      ...
      const yahooFinanceUrl = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(symbol)}?interval=1m&includePrePost=false&t=${Date.now()}`;
...
const handleBuyStock = async (e) => {
  ...
  try {
      ...
      const walletResponse = await fetch('http://localhost:5000/api/update/tradewallet', {
          method: 'PUT',
          headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${token}`
